...
(window => {
  ...
  const { screen: { width, height }, navigator: { language }, location: { hostname, pathname, search }, localStorage, document, history, } = window;
  ...
  const post = (url, data, callback) => {
    ...
    if (cache)
      req.setRequestHeader('x-umami-cache', cache);
  ...
  const trackView = (url = currentUrl, referrer = currentRef, uuid = website) => {
    collect('pageview', assign(getPayload(), {
      website: uuid,
      url,
      referrer,
    }));
  ...
  const trackEvent = (event_value, event_type = 'custom', url = currentUrl, uuid = website) => {
    collect('event', assign(getPayload(), {
      website: uuid,
      url,
      event_type,
      event_value,
